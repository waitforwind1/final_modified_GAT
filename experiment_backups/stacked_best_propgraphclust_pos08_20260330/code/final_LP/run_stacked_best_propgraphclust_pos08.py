...
HYBRID_RUNNER = ROOT / "final_LP" / "run_hybrid_gate_lp.py"
...
def parse_args():
    ...
    parser.add_argument(
        "--output",
        default=str(ROOT / "final_LP" / "stacked_best_propgraphclust_pos08_selected_20run.txt"),
    )
    parser.add_argument(
        "--prefix",
        default="stacked_best_propgraphclust_pos08",
        help="Prefix for intermediate files under final_LP/.",
    )
...
def parse_result_file(path):
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if not line:
            continue
        ds, avg, std, avg_time, paper, delta_paper, target, delta_target = line.split("\t")
        rows[ds] = {
            "dataset": ds,
            "avg": float(avg),
            "std": float(std),
...
def search_best(job_name, datasets, watched_dataset, target, max_attempts, args):
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if output_path.exists():
            ...
            print(
                f"\n=== Running {job_name} attempt {attempt}/{max_attempts} ===",
                flush=True,
            )
        ...
        print(
            f"{DISPLAY[watched_dataset]} attempt {attempt}: {row['avg']:.3f} "
            f"(target {target:.3f}, best {best_row['avg']:.3f})",
            flush=True,
        )
        if row["avg"] >= target:
            print(
                f"Target reached for {DISPLAY[watched_dataset]} on attempt {attempt}.",
                flush=True,
            )
            break
    ...
    with path.open("w", encoding="utf-8") as f:
        for ds in ORDER:
            ...
            f.write(
                ...
                f"{row['paper']:.3f}\t{row['delta_paper']:+.3f}\t"
                f"{row['target']:.3f}\t{row['delta_target']:+.3f}\n"
            )
...
def main():
    ...
    for ds in ORDER:
        ...
        print(
            ...
            f"[mode={mode}, attempt={attempt}, source={Path(row['source_file']).name}]",
            flush=True,
        )
